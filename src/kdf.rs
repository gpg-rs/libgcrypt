...
pub fn s2k_derive(token: Token, algo: DigestAlgorithm, iter: u32,
    ...
    match (iter, salt.is_some()) {
        (x, true) if x != 0 => {
            derive(token, KDF_ITERSALTED_S2K, algo.raw() as i32, iter, pass, salt, key)
        }
        (_, true) => {
            derive(token, KDF_SALTED_S2K, algo.raw() as i32, iter, pass, salt, key)
        }
        _ => {
            derive(token, KDF_SIMPLE_S2K, algo.raw() as i32, iter, pass, salt, key)
        }
    }
}
...
pub fn pbkdf1_derive(token: Token, algo: DigestAlgorithm, iter: u32,
    ...
    derive(token, KDF_PBKDF1, algo.raw() as i32, iter, pass, Some(salt), key)
}

pub fn pbkdf2_derive(token: Token, algo: DigestAlgorithm, iter: u32,
                     pass: &[u8], salt: &[u8], key: &mut [u8]) -> Result<()> {
    derive(token, KDF_PBKDF2, algo.raw() as i32, iter, pass, Some(salt), key)
}

pub fn scrypt_derive(token: Token, n: u32, p: u32,
                     pass: &[u8], salt: &[u8], key: &mut [u8]) -> Result<()> {
    derive(token, KDF_SCRYPT, n as i32, p, pass, Some(salt), key)
}
